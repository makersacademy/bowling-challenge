...
Bowling.prototype.roll = function (pins) {
    ...
    if (this.frameCount === 10 && this.ballCount === 1 && pins === 10) { // ball 1 strike in 10th frame
      ...
      console.log("this frame is: " + this.frameCount);
      ...
      console.log("pinstore = " + this.pinStore);
      this.counter += 1;
      this.ballCount = 2;
    } else if (this.frameCount === 10 && this.ballCount === 2 && pins === 10) { // ball 1/2 or ball 2 only strike in 10th frame
      this.rolls[this.counter] = pins;
...
Bowling.prototype.score = function () {
  ...
  for (var frame = 0; frame < 10; frame++) {
    ...
    } else if (frame === 9 && this.rolls[rollsIndex] + this.rolls[rollsIndex + 1] >= 10) {
      matchResult += this.rolls[rollsIndex] + this.rolls[rollsIndex + 1] + this.rolls[rollsIndex + 2];
      // 10th frame 1st/2nd ball STRIKE or SPARE plus extra ball.

    // } else if (frame === 9 && this.rolls[rollsIndex] >= 0) {
    //   matchResult += this.rolls[rollsIndex + 0] + this.rolls[rollsIndex - 1];
    //   // 10th frame 3rd ball.
