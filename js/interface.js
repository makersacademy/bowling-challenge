$(document).ready(function() {
  ...
  $('.scorebuttons').on('click', function() {
    ...
    showScore()
    if (game.currentframe.isFrameOpen() || game.currentframe.isFinalFrame()) {
      frametext = game.currentframenumber
    } else {
      frametext = game.currentframenumber + 1
    }
    ...
    if (game.currentframe.isFinalFrame() && !game.currentframe.isFrameOpen() &&
      ...
      $('#enter_pins_message').text("How many did you get for your strike bonus?")
    }

    if (game.currentframe.isFinalFrame() && !game.currentframe.isFrameOpen() &&
      game.currentframe.hasSpare()) {
      $('#enter_pins_message').text("How many did you get for your spare bonus?")
    }
