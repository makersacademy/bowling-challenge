...
class Bowling {
  ...
  roll(pins) {
    ...
    if (this.isFrameComplete() && this.frames.length !== this.MAX_FRAMES - 1) {
      this.completeFrame();
      this.refreshFrame();
    }
    ...
    if (this.frames.length === this.MAX_FRAMES - 1) {
      if (this.isStrike(this.currentFrame)) {
        this.framesScores[this.MAX_FRAMES - 1] = this.getFrameScore();
        if (this.currentFrame.length === this.MAX_TRIES + 1) {
        ...
        this.framesScores[this.MAX_FRAMES - 1] = this.getFrameScore();
        if (this.currentFrame.length === this.MAX_TRIES) {
          this.frames.push(this.currentFrame);
        }
      }
    }
  ...
  calculateBonusPoints(pins) {
    ...
    if (this.frames.length > 1 && this.isStrike(this.frames[this.frames.length - 2])) {
