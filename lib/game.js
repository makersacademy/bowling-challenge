...
class Game {
  ...
  totalScore () {
    ...
    this.frames.forEach((frame, index) => {
      ...
      } else if (frame.checkStrike()) {
        if (index === 8) {
          totalscore += frame.score() + this.frames[index + 1].rolls[0] + this.frames[index + 1].rolls[1];
        } else {
          totalscore += frame.score() + this.frames[index + 1].rolls[0] + (this.frames[index + 1].checkStrike() ? this.frames[index + 2].rolls[0] : this.frames[index + 1].rolls[1]);
        }
