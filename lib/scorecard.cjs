...
class ScoreCard {
  static get NumberOfPins() {
    return 10;
  }
  static get NoOfFrames() {
    return 10;
  }
  ...
  formatScores() {
    ...
    this.frames.map((frame, frameIdx) => {
      currentFrameScore = this.#currentFrameScore(frameIdx, frame);
  ...
  gameComplete() {
    return (this.frames.length == ScoreCard.NoOfFrames &&
        this.frames.at(-1).frameComplete() ? true : false );
  }
  // Private methods
  ...
  #bonusFrameScore(frame, frameIdx) {
    ...
    return (this.#bonusPoints(frameIdx, bonusRolls) > 0 ?
      ScoreCard.NumberOfPins + this.#bonusPoints(frameIdx, bonusRolls) : 0);
  }
  ...
  #currentFrame() {
    if (this.#newFrame()) {
      this.frames.length + 1 == ScoreCard.NoOfFrames ?
  ...
  #newFrame() {
    ...
    if (this.frames.length == ScoreCard.NoOfFrames) {
      return false;
    }
    if (this.frames.at(-1).frameComplete()) {
      return true;
    }
    return false;
  }
