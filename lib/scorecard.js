...
class Scorecard {
  ...
  async getSpareBonusRoll () {
    ...
    rl.close()
    return new Frame(parseInt(roll1), 0);
  }
  ...
  checkForBonus () {
    if (this.frame > 0 && this.frame < (MAX_ROUNDS - 1) && this.allFrames[this.frame - 1].isStrike()) {
      this.calculateStrikeBonus();
    } else if (this.frame > 0 && this.frame < (MAX_ROUNDS - 1) && this.allFrames[this.frame - 1].isSpare()) {
