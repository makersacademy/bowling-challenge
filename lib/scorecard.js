...
class Scorecard {
  ...
  addToFramesArray = () => {
    if (this.framesArray.length < this.rulesClassInstance.maxFrames - 1) {
      ...
      if (!this.#strikeOrSpare() && this._frame.length === 2) {
        this.#pushFrameAndReset();
  ...
  #lastFrame = () => {
    return this.framesArray.length === this.rulesClassInstance.maxFrames - 1;
  };

  #strikeOrSpare = () => {
    return this.rulesClassInstance.isStrike(this._frame) || this.rulesClassInstance.isSpare(this._frame);
  };
}
