class Scorecard {
  ...
  checkSpecials(frame) {
    if (frame[0] === 10) {
      return "strike";
    } else if (this.sum(frame) === 10) {
      return "spare";
    }
  }

  checkForPerfectOrGutter() {
    // This flattens the entire frames array and collects all the numbers in a set object. Sets only hold
    // unique values and disregard if the same value is passed inside them
    const uniques = new Set(this.frames.flat());
    // This means our set has more than 1 unique value, meaning the player has scored something else than 0 or 10
    // which results in a regular game
    if (uniques.size > 1) {
      return "Regular game";
    } else if (uniques.size === 1) {
      // If there is only one unique value these conditionals check to determine if that value is 10 or 0
      if (uniques.has(10) === true) {
        return "Perfect game";
      } else if (uniques.has(0) === true) {
        return "Gutter game";
      }
    }
  }
  ...
  calculateScore() {
    ...
    // Bonus scores are covered in a different method since they are counted different to the game
    if (this.addBonusScore() !== undefined) {
      score += this.addBonusScore();
    }
    return score;
  }
  ...
  // Resets the game if the user is playing through node
  resetGame() {
    this.frames = [];
  }
}
