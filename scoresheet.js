class Scoresheet {
  ...
  #strikeBonus(frameNum) {
    ...
    if (frameNum === 9) {
      bonus +=
        this.frames[frameNum].secondRoll() + this.frames[frameNum].bonusRoll();
    } else if (
      frameNum === 8 &&
      this.frames[frameNum + 1].isStrike() === true
    ) {
      bonus +=
        this.frames[frameNum + 1].firstRoll() +
        this.frames[frameNum + 1].secondRoll();
    } else if (this.frames[frameNum + 1].isStrike() === true) {
      bonus +=
        this.frames[frameNum + 1].firstRoll() +
        this.frames[frameNum + 2].firstRoll();
    } else {
      bonus +=
        this.frames[frameNum + 1].firstRoll() +
        this.frames[frameNum + 1].secondRoll();
