describe("Frame", function() {
  ...
  describe('Returning Display Symbols', function() {
    it("should return numbers on normal frame", function() {
      frame.addScore(5);
      frame.addScore(1);
      expect(frame.getDisplaySymbols()).toEqual({first: 5, second: 1})
    });
    it("should return whitespace/strike on strike", function() {
      frame.addScore(10);
      expect(frame.getDisplaySymbols()).toEqual({ first: '\u00A0', second: 'X' })
    });
    it("should return number/spare on spare", function() {
      frame.addScore(3);
      frame.addScore(7);
      expect(frame.getDisplaySymbols()).toEqual({ first: 3, second: '/' })
    });
  });
});
