...
class Frame {
  ...
  _normalFrame(pins) {
    this._rollOne === null ? this._normalFirstRoll(pins) : this._normalSecondRoll(pins)
  }
  ...
  _isBonusRoll(pins) {
    this._rollTwo === null ? this._finalSecondRoll(pins) : this._finalThirdRoll(pins)
  }

  _finalSecondRoll(pins) {
    this._rollTwo += pins
    let frameTotal = this._rollOne + this._rollTwo
    frameTotal >= 10 ? this._remainingPins = this.TOTAL_PINS : this._isFull = true
  }
