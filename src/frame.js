...
Frame.prototype.inputRoll = function (pins) {
  if (!this.canRoll()) {
    throw new Error('Cannot Roll, frame has ended')
  } else {
...
Frame.prototype.inputExtra = function (pins) {
  if (this._extrasRequired > 0 && !this.canRoll()) {
...
Frame.prototype.final2 = function () {
  joinArr = this.rolls.concat(this._extras)
  if (joinArr[1] === 10) {
    return 'X'
  } else if (this.rolls.reduce((a, b) => a + b, 0) === 10 && joinArr[0] !== 10) return '/'
  return joinArr[1]
}

Frame.prototype.final3 = function () {
  joinArr = this.rolls.concat(this._extras)
  if (joinArr[2] === 10) return 'X'
  return joinArr[2]
}
