...
Scorecard.prototype.currentScores = function () {
  return this._scores
}

Scorecard.prototype.currentFrames = function () {
  return this._frames
}
...
Scorecard.prototype.throw = function (pinsKnockedDown) {
  ...
  if (itsTheFirstThrowOfAFrame && theyKnockDownTenPins) {
    this.recordsAStrike()
  } else {
    this.recordsNumberOf(pinsKnockedDown)
  }
...
Scorecard.prototype.thenTalliesTheScoreAtTheEndOfEachFrame = function () {
  ...
  for (i = 0; i < numberOfThrowsSoFar; i++) {
    ...
    var theSecondScoreOfTheFrame = (this._scores[i])
    var thatScore = (this._scores[i])
    ...
    if (itsTheFirstScoreOfTheFrame) {
      hold += thatScore
    } else {
      hold += theSecondScoreOfTheFrame
      if (theyScoredAStrikeThisFrame) {
        hold += theScoreFromTheFirstThrowOfTheNextFrame
        if (thatWasAlsoAStrike) {
          hold += theScoreFromTheThrowAfterThat
        } else {
          hold += theScoreFromTheSecondThrowOfTheNextFrame
        }
      } else if (theyScoredASpareThisFrame) {
        hold += theScoreFromTheFirstThrowOfTheNextFrame
      }
      if (!isNaN(hold)) {
        this.recordsTheTallyAtTheEndOfThisFrameAs(hold, i)
      }
    }
  }
}
